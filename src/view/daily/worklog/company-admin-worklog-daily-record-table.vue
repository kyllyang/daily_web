...
export default {
  ...
  methods: {
    handleCreate () {
      this.$router.push({ name: 'company_admin_worklog_daily_record_edit' })
    },
    handleModify () {
      if (this.$refs.dataTable.getSelection().length === 1) {
        this.$router.push({ name: 'company_admin_worklog_daily_record_edit', params: { id: this.$refs.dataTable.getSelection()[0].id } })
      } else {
        this.$Modal.warning({
          title: '警告',
          content: '请选择一条记录！'
